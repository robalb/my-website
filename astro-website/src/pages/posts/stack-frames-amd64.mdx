...
The `push` and `pop` instructions offer the simpliest interaction with the stack:
...
There is a confusing element here: when we put something onto the stack, therefore
growing the stack, we are moving towards lower addresses of memory.
