...
- In the [first part](./ebpf-fingerprinting-1), which you are reading right now, I provide a background on TCP fingerprinting, and discuss some implementation strategies.
- In the [second part](./ebpf-fingerprinting-2) I describe the actual development and testing of an XDP eBPF program capable
...
It's just a feature of the linux kernel, which introduces special event-driven programs that can run 
kernel side,
and [special data structures](https://docs.cilium.io/en/stable/reference-guides/bpf/architecture/#maps)
that both epbf and user space programs can access and share.
...
the [cilium docs on ebpf](), and the linux docs in particular.

There are also a lot of tooling and libraries that simplify the process of writing and executing 
eBPF programs. In particular, for integrating eBPF from C programs there is the [ebpf c, lib] which provides a list of [example projects]
and a template to get started. But there are also Golang projects with a similar usage, like [go cilium]

With these tools, an eBPF program can be written in C, by importing some dedicated libraries. It can then be 
compiled into an ELF using Clang, together with autogenerate boilerplate code in either C or Go that will
provide the functions to launch this eBPF program from inside our own program.

Don't be fooled by the ELF format though, and eBPF program is just a sequence of bytes that can only be interpreted 
by a custom emulator-like construct living inside the kernel. You execute a program by passing its bytes to a dedicated
`ebpf` syscall, and performing some extra steps after that. 

These tools abstract away all this work, allowing us to focus on C code. The resulting Object files are just a convenient 
format to store both the program and its metadata, like debug symbols, in a standardized and well-known format.

The end-result of this project needs to be in GO, so i'll focus on [cilium ebpf], for now.
We can get take care of all the boilerplate simply by following the [cilium ebpf getting started guide](https://ebpf-go.dev/guides/getting-started/#the-go-application), which defines an example eBPF program
and a Golang program that lauches it.

[[[ (note: this whole block can be removed if we choose to keep a single article)

The development of this code is described in the [second part] of the article.

---

This is the second part of my series documenting my experience implementing fast TCP fingerprinting 
in a golang webserver, using eBPF.  
...
taken from its TCP SYN handshake.

[diagram]

We are going to use cilium ebpf, wich provides an easy way to interact with an eBPF program from GO.
We can take care of all the boilerplate simply by following the [cilium ebpf getting started guide](https://ebpf-go.dev/guides/getting-started/#the-go-application), which defines an example eBPF program
and a Golang program that launches it.

]]]

### The golang side

There isn't anything particularly interesting in the user space side of our project.
...
and with our current system we can define which one we want to use using the 
`SEC` macro.

The first question we need to figure out is what program type will give us fast access to the data we need.
Strictly speaking, there are only three possible candidates:

#### approach 1: Kernel probes

[Kernel probes] allow us to hook into almost all functions inside the kernel.
In order to get our TCP SYN, we could hook into one of the core functions that handle TCP connections.

There are [detailed guides](https://devops-insider.mygraphql.com/zh-cn/latest/kernel/network/socket/listen-queues/ref/scaling-linux-services-before-accepting-connections.html#from-syn-to-accept) that describe the part of the networking stack we are interested about. 
Regarding TCP specifically, we know that TCP SYN packets directed to a listening socket are handled by the `tcp_conn_request()` function, and put into a dedicated SYN backlog queue.

[diagrams of SYN BACKLOG and acept queue]

Hooking into `tcp_conn_request` would give us access to both the SYN packet we need and the socket it's directed to, which is exactly what we need:
...
writing our eBPF with [CORE](https://docs.ebpf.io/concepts/core/), which maintains compatibilty across kernel versions. But this is probably 
not the right path to follow.

#### approach 2: tracepoints

[Tracepoints] are more stable than kernel probes, and more lightweight compared to packet hooks for the simple reason that they get triggered with 
...
This leaves us with [XDP programs], which are the intended way to handle network packets.
...
int find_tcp_syns(struct xdp_md *ctx) {
  ...
  // check that the destination addr matches with the address of our server.
  // dst_ip is provided at execution time by the golang project, already in network byte order.
  if (ip->daddr != dst_ip)
    return 0;
  ...
  // check that the destination port matches with the listening port of our server.
  // dst_port is provided at execution time by the golang project, already in network byte order.
