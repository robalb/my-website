...
The goal of this project is to implement a web server that echoes back to the client
its TCP fingperprint.
It can be useful to approach this problem from first principles, starting from understanding how web servers work 
under the hood.
...
This is certainly not a production ready webserver, but it servers the purpose of being extremely simple.
...
The previous paragraph might have provided a good introduction to what eBPF is,
and what can be done with it:
It's just a feature of the kernel, which introduces special event-driven programs that can run 
kernel side,
...
This topic is very well documented, and has several [introductory guides](https://ebpf.io/what-is-ebpf/#introduction-to-ebpf) available.
