...
[This project was developed as part of a larger anti-bot solution I've been
working on lately, in my attempt to stay ahead on the ever-evolving arms race between scraping and 
anti-scraping techonologies.]

Implementing such a system offers interesting technical challenges that span different domains, 
which is why I believe this is an experience worth sharing.  
I split this article in two parts. In the first one, which you are reading right now,
I provide a background on TCP fingerprinting, and discuss some implementation strategies.

In the second part I describe the actual development and testing of a XDP eBPF program capable
of handling 1 Million cuncurrent requests.
...
In order to fully understand what we want to do, I believe it's useful to start from what we know about web servers,
and analize our problem from first principles.
...
An HTTP/1 web server is just a light layer on top of a TCP connection, and can be implemented in just a few lines of C code.
...
The code is not even remotely sufficient to run a webserver in production,
but it servers the purpose of showing the inner working of an HTTP/1 connection.
