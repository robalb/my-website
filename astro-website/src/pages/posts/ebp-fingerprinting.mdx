...
Over ethernet we usually cannot send more than 1500 bytes over a single transaction,
...
It's clear that network connections can be complicated, and we can't expect to learn 
everything from a single value. But at least it can provide some insight,
and some entrpy for a fingerprinting system.
...
its TCP fingerprinting data. For this to work,
our webserver needs to directly access the TCP SYN data of the connection it receives.
How do we do it?
...
Sadly, if you skim at the contents of that struct, you'll see that it only contains 
...
And indeed we can, using the for example the LibPcap library. Out of the box, the library 
offesr the a very simple system for filtering and capturing network packets into a 
userspace buffer.

A simple approach to implement our webserver could work like this:

First, we start LibPCAP in a spearate thread, capturing TCP SYN packets that are
directed to our webserver.  
When we receive a packet, we store it in a hashmap with a key composed of 
`client IP + client PORT`.
Then, when our webserver receives a request, we lookup the 
