...
Any kind of information can be encoded in a sequence of bits, however the examples in
this article will only focus on text and integers.
...
a group of hexadecimal numbers have name:


 a 4 bits (for example: `f`) -> nibble
  8 bits ff -> byte
  16 bits ffff -> word
  32 bits ffffffff -> dword (a double word)
  54 bits ffffffffffffffff -> qword (a quadruple word)
...
[You probably know](https://www.joelonsoftware.com/2003/10/08/the-absolute-minimum-every-software-developer-absolutely-positively-must-know-about-unicode-and-character-sets-no-excuses/)
 that there are different ways to encode text. The easiest one is ascii,
...
it the same order as you normally write it.
