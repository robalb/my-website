...
It could really be any kind of information, but we are only interested in
text, and integers.

We are now in the uncomfortable position were we have to talk about 
sequences of bits,
which is something that circuits understand very well, but not humans. <br/>
For example, can you tell the difference between
`1101010101110` and `1101010101110` ?

<spoiler text="Show answer">
Ok, the two sequences are identical, but I bet you could not see that in a glance.
</spoiler>
...
You probably know that text is usually stored as a sequence of bytes,
where every byte represent a letter.
for example, the letter 'a' will become `0x61`

### numbers

Integers on the x86-64 architecture are stored in little endian format.

that means that when you write a number, let's say:
`1337`
