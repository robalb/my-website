...
what [this article does](https://web.archive.org/web/20080607055623/http://www.ibm.com/developerworks/library/pa-dalign/), so check it out. We'll only focus on how memory alignment impacts
...
As a consequence, when we visualize memory in a hexdump we always start from addresses multiples
of 8 or 16, so that data will fit properly in a row.

this snippet of code is the same as the previous one, but with the
hexdump starting at unaligned memory addresses, which makes everything hard to follow

<StackEmbed client:load align={true} /><br/>
